<?php
    ...
    class User{
        ...
        public function getFriendList($userLoggedIn){

            $query = mysqli_query($this->conn, "SELECT friend_array FROM users WHERE username = '$userLoggedIn'");
            $row = mysqli_fetch_array($query);
            $friend_array = $row['friend_array'];
            $friend_split = explode(",", $friend_array);
            
            $str = "";

            foreach($friend_split as $item){
                $username = $item;
                ...
                $profile_pic = $names['profile_pic'];

                if($first_name != ""){
                    $str .= '<div class="friends_list_container">

                <img src="'.$profile_pic.'" alt="">

                <p><a href="'.$username.'">'.$first_name.' '.$last_name.'</a></p>
